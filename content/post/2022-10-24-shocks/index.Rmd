...
# 
# x <- df1 %>%
#   filter(st_cd_fips == 101) %>%
#   arrange(year)
# 
# x$DemVS_lag <- lag(x$DemVotesMajorPercent, 1)
# 
# x <- x %>% select(!DemVotesMajorPercent)
# 
# data$DemVS_lag <- left_join(data, x, by = c(year, st_cd_fips))
# 
# for (i in 100:max(df1$st_cd_fips)){
#   x <- df1 %>%
#   filter(st_cd_fips == i) %>%
#   arrange(year)
#   df1$DemVS_lag <- lag(x$DemVotesMajorPercent,  1)
# }
```
